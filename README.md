...
- **准确识别环境消息**：只处理 user 消息中，`internal_chat_message_metadata_passthrough.content_item_kinds` 对应位置为 `environments.environment_context` 的文本。按原 content 索引匹配，图片等非文本项不会造成错位。用户示例、assistant 消息、工具输出保持原样；JSON 中的 `\u003c` 等转义不影响识别。没有类型标记的旧客户端无法与用户粘贴内容可靠区分，因此不凭 XML 前缀猜测，不会为陌生会话启用改写。
...
- **重启与淘汰**：首次映射保存在 `data_dir/env-timezone/v1/`，按目标时区、会话和原环境文本的 SHA-256 摘要隔离，只存摘要、日期和识别状态，不保存原环境文本或凭据。新映射同步写入成功后才返回改写结果。重启、内存缓存淘汰、禁用后重新启用会重用已有映射；状态读取或写入失败时记录日志，本次请求原样转发。
- **状态目录维护**：状态文件独立于抓包记录，不受 `max_records`、`max_disk_mb` 或清空抓包记录影响，也不按 TTL 删除，以免重新换算历史。需随数据目录持久化、备份；只由一个插件进程使用同一目录。磁盘占用随会话和不同环境片段数量增长。删除状态或更换数据目录会失去历史映射；旧版仅在内存中的映射无法在升级时自动恢复。
- **在哪里看**：请求详情的「① 客户端请求」保留客户端原文，并列出本次改写和追加的说明；「② 发往上游」是处理后的版本。
- 只处理经 CPA 转发的请求体；客户端系统时区、工作目录、shell、权限和工具输出（如 `date` 结果）不受影响。
...
| `envctx.go` / `envstate.go` | `rewrite_env_timezone`：识别环境消息、改写日期与时区、追加当前日期更新及持久化历史映射 |
